...
class Q1(unittest.TestCase):
    ...
    def test_feature_extractor2(self):
        ...
        d = extract_image_features(image, feats)
        actual = {k: round(d[k], 2) for k in d} 
        expected = {'offset': 1.0, 'black': 0.63, 
...
class Q4(unittest.TestCase):
    ...
    def test_pixel_pattern_extractor(self):
        ...
        feats = extract_pixel_pattern_features(image, 130,
                                               [(0,0), (0,1), (1,0), (1,1)])
        output = {k: round(feats[k], 2) for k in feats}
        expected = {'offset': 1.0, 'p_0111': 0.0, 'p_1111': 0, 'p_0011': 0, 
                    'p_1011': 0.0, 'p_0101': 0, 'p_1101': 0.0, 
                    'p_0001': 0.69, 'p_1001': 0, 'p_0110': 0, 
                    'p_1110': 0.0, 'p_0010': 0.69, 'p_1010': 0, 
                    'p_0100': 0.69, 'p_1100': 0, 'p_0000': 1.39, 
                    'p_1000': 0.69}
        self.assertEqual(expected, output)
        feats = extract_pixel_pattern_features(image, 130,
                                               [(0,0), (0,2), (2,1)])
        output = {k: round(feats[k], 2) for k in feats}        
        expected = {'offset': 1.0, 'p_011': 0.0, 'p_111': 0, 'p_001': 1.39, 
                    'p_101': 0, 'p_010': 0.69, 'p_110': 0.0, 'p_000': 0, 
                    'p_100': 0.69}
        self.assertEqual(expected, output)
